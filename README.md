...
Criação do Repositório: Você deve possuir um repositório dedicado exclusivamente para a documentação técnica da disciplina de Laboratório de Sistemas Operacionais e Redes no seu GitHub.
Configuração da Página Inicial: Certifique-se de que o repositório possua um arquivo README.md como página de índice apontando para todos os seus relatórios e avaliações.
Entrega Oficial: Copie o conteúdo deste arquivo de template, salve como Avaliacao1.md em seu repositório, responda às questões abertas diretamente no arquivo e insira as capturas de tela (prints) correspondentes nos locais demarcados. O envio final será feito através do formulário do Google Forms, fornecendo o link direto deste arquivo .md publicado no GitHub.
Execução de Comandos: Para fins de conformidade técnica e validação das permissões, execute todas as tarefas de criação e controle com privilégios administrativos usando sudo apenas onde for estritamente necessário.
...
🖼️ INSERIR PRINT DA VM IMPORTADA ATIVA NO SEU VIRTUALBOX AQUI (A captura de tela deve mostrar a janela do VirtualBox com a VM UbuntuAvaliacao1 em execução e o console de login ativo).
...
Sua resposta teórica:Baixar a iso do SO desejado, após isso importar dentro do VirtualBox. O .iso é mídia/imagem para instalar um SO; .ova é um appliance empacotado, normalmente contido disco virtual e configurações da VM, facilitando portabilidadee reprodução de um ambiente já preparado.
...
Usuário: luiz | Senha: luiz13
Usuário: flavio | Senha: flavio22
Usuário: augusto | Senha: augusto70
Usuário: renan | Senha: renan14
🖼️ INSERIR PRINT DO TERMINAL EXECUTANDO A CRIAÇÃO DE UM DOS USUÁRIOS E ASSOCIAÇÃO AO GRUPO AQUI (A captura de tela deve demonstrar a saída dos comandos aplicados no console).
...
Sua resposta teórica: O comando adduser é mais seguro e menos propenso a erros humanos do que o useradd porque ele valida as entradas interativamente e define permissões restritivas por padrão. o adduser é um utilitário de nível mais alto, interativo e amigável, que normalmente automatiza criação da home, senha e configurações padrão. Useradd é de nível mais baixo e exige que o administrador informe mais opções explicitamente.
...
🖼️ INSERIR PRINT EXIBINDO AS ÚLTIMAS ENTRADAS DOS COMANDOS DE AUDITORIA AQUI (A captura de tela deve exibir a saída do terminal comprovando a existência dos usuários e o vínculo com o grupo).
...
Sua resposta teórica: O getent consulta as bases configuradas no NSS, por isso ele não está limitado conceitualmente à leitura daquele arquivo local; Ele tambem pode consultar as fontes de contas/grupos configuradas no sistema.
...
🖼️ INSERIR PRINT DO ARQUIVO DE CADASTRO DE UM DOS USUÁRIOS EXIBIDO COM O COMANDO CAT AQUI (A captura de tela deve demonstrar o conteúdo do arquivo e o caminho completo onde ele foi salvo).
...
Sua resposta teórica: [Substitua este texto pela sua resposta técnica]
...
🖼️ INSERIR PRINT DO TERMINAL EXECUTANDO A CRIAÇÃO DO DIRETÓRIO AQUI (A captura de tela deve demonstrar o comando executado no terminal e a verificação do diretório /eleicoes listado).
...
Solicitar interativamente que o aluno digite seu próprio nome completo.
Gerar (ou sobrescrever) um arquivo de saída chamado /eleicoes/presidenciaveis2026.txt.
Escrever o cabeçalho no arquivo contendo o nome do aluno que o executou e a data da operação.
Realizar uma busca automatizada por todos os arquivos cadastroeleitoral.txt salvos nas homes dos usuários.
Extrair e anexar o conteúdo de cada um dos cadastros eleitorais encontrados para dentro de /eleicoes/presidenciaveis2026.txt.
...
🖼️ INSERIR PRINT DO SCRIPT ESCRITO SENDO EXIBIDO COM O COMANDO CAT NO TERMINAL AQUI (A captura de tela deve mostrar o código completo do script dentro do console do Ubuntu Server).
...
🖼️ INSERIR PRINT EXIBINDO AS PERMISSÕES DE EXECUÇÃO JÁ ALTERADAS COM O COMANDO LS -L AQUI (A captura de tela deve demonstrar claramente a modificação da máscara do arquivo de script, contendo a flag 'x').
...
🖼️ INSERIR PRINT DO TERMINAL MOSTRANDO A EXECUÇÃO DO SCRIPT E A EXIBIÇÃO DO RELATÓRIO FINAL COM O COMANDO CAT AQUI (A captura de tela deve exibir o input do seu nome, a conclusão do script e a leitura completa do arquivo presidenciaveis2026.txt contendo o cabeçalho com seu nome e as informações em JSON dos candidatos).
...
🖼️ INSERIR PRINT DO TERMINAL EXECUTANDO OS FILTROS DE LINHAS INICIAIS E FINAIS AQUI (A captura de tela deve demonstrar os comandos de filtragem aplicados e os respectivos retornos no console).
...
🖼️ INSERIR PRINT DO TERMINAL COMPARANDO O DIRETÓRIO DE TRABALHO APÓS CADA UM DOS COMANDOS AQUI (A captura de tela deve exibir a execução de 'su luiz' seguido do comando 'pwd', e de 'su - luiz' seguido de 'pwd', demonstrando a mudança de caminhos).
